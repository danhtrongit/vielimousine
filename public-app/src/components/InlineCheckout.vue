...
import type { CreateOrderRequest, CreateOrderResponse, CouponValidateResponse } from '@/api/types';
...
async function submitOrder(ev: Event) {
  ev.preventDefault();
  errorMsg.value = '';
  if (!selection.roomId) { errorMsg.value = 'Vui lòng chọn phòng.'; return; }
  if (!form.name.trim() || !form.phone.trim()) {
    errorMsg.value = 'Vui lòng nhập họ tên và số điện thoại.';
    return;
  }
...
<template>
  <section v-if="room" class="vh-checkout" data-vh-checkout>
    <h2 class="vh-section-title">
      <i class="pi pi-check-circle" /> Hoàn tất đặt phòng
    </h2>
    <div class="vh-progress">
      <span class="vh-step vh-step-done">1. Phòng đã chọn</span>
      <span class="vh-step vh-step-active">2. Thông tin & Thanh toán</span>
    </div>
    ...
    <form class="vh-checkout-form" novalidate @submit="submitOrder">
      <fieldset class="vh-fieldset">
        ...
        <div class="vh-field">
          <label>Ghi chú</label>
          <Textarea v-model="form.customer_note" rows="2" placeholder="Yêu cầu đặc biệt..." fluid />
        </div>
      </fieldset>
