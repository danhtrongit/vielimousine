...
<table role="presentation" cellpadding="0" cellspacing="0" width="100%" style="border-collapse:separate;border:1px solid #e5e7eb;border-radius:6px;margin:0 0 12px;">
  ...
  <tr><td style="padding:6px 12px;color:#6b7280;">Số tiền vừa thu</td><td style="padding:6px 12px;font-weight:600;color:#007A3D;"><?php echo esc_html((string) ($ctx['payment_amount'] ?? '')); ?></td></tr>
...
<?php if (!empty($ctx['admin_url'])): ?>
  <p style="margin:16px 0;text-align:right;">
    <a href="<?php echo esc_url((string) $ctx['admin_url']); ?>" style="display:inline-block;background:#00A651;color:#ffffff;padding:8px 16px;border-radius:6px;text-decoration:none;font-weight:600;font-size:13px;">Xem chi tiết →</a>
  </p>
<?php endif; ?>
