...
use Vie\Container;
use Vie\Service\Pricing\SurchargePriceBulkService;
use Vie\Support\ResponseEnvelope;
use Vie\Support\Validator;
use Vie\Validation\Schemas\SurchargePriceBulkValidation;

final class SurchargePriceBulkController
{
    public static function bulk(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        $v = Validator::validate($data, SurchargePriceBulkValidation::rules());
        if ($v->fails()) {
            return ResponseEnvelope::error($v->errors(), 422);
        }
        $cross = SurchargePriceBulkValidation::crossValidate($v->validated());
        if ($cross !== []) {
            return ResponseEnvelope::error($cross, 422);
        }

        try {
            $svc    = Container::get(SurchargePriceBulkService::class);
