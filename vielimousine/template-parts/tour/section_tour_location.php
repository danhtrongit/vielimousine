...
if ( $acf_fc_layout !== 'section_tour_location' ) {
	return;
}
$title = $args['title'] ?? '';
...
<section class="section tour-category tour-category-location relative">
    <div class="group-heading">
        <?php if($title){
            echo '<h2 class="heading-title">'. $title .'</h2>';
        }
        if($link_all){
            echo '<a href="'. $link_all .'" class="btn-link">More Tours ></a>';
        } ?>
    </div>
    <?php 
    if($sl_tour_cat){ 
