...
function observePrintButton() {
  const buttons = [{ selector: ".share-intent-print", title: "Print" }];
  let allFixed = true;
  buttons.forEach(({ selector, title }) => {
    const button = document.querySelector(selector);
    if (button) {
      if (!button.title || button.title === "undefined") {
  ...
  if (allFixed) return;
  const shareContainer = document.querySelector("[data-social-share]");
  const observeTarget = shareContainer || document.body;
