...
const $locale = (l10n2) => new Proxy(l10n2, {
   ...
   */
  get(self2, marker) {
    return self2.lang && self2[self2.lang][marker] || marker;
  }
});
const l10n = $locale({});
/**
...
class Data {
  ...
  async render(slot) {
    ...
    if (el[slot] && el[slot]._) {
      ...
      el[slot] = { _: self2[slot], load: self2[slot].load, unload: self2[slot].unload };
      self2[slot] = new Compo(self2[slot].ns, self2[slot].tag, self2[slot].name, self2[slot].props);
      el[slot].load();
    }
  }
  /**
...
class base {
  ...
  wrap(method) {
    ...
    return function() {
      method.call(self2, ...arguments, this);
    };
  }
}
/*!
...
function History(_ref) {
  ...
  const getPathValues = (urlOverride) => {
    ...
    const pathArray = location.pathname.slice(1).split("/").filter((part2) => part2 !== "");
...
function ignoreMousedisappear(handler, { ignoreLeaveWindow = false, ignoreReappear = false } = {}) {
  return function leaveEventHandler(eLeave, ...rest) {
    ...
    setTimeout(function leaveEventDebouncer() {
      ...
      if (!ignoreReappear) {
        $$1(document).one("mouseenter", function reenterEventHandler(eReenter) {
          if (!$$1(eLeave.currentTarget).has(eReenter.target).length) {
...
var MediaQuery = {
  ...
  _init() {
    ...
    var self2 = this;
    var $meta = $$1("meta.foundation-mq");
    if (!$meta.length) {
      $$1('<meta class="foundation-mq" name="foundation-mq" content>').appendTo(document.head);
    }
    var extractedStyles = $$1(".foundation-mq").css("font-family");
    var namedQueries;
   ...
   */
  _watcher() {
    $$1(window).on("resize.zf.trigger", () => {
      var newSize = this._getCurrentSize(), currentSize = this.current;
      if (newSize !== currentSize) {
        this.current = newSize;
        $$1(window).trigger("changed.zf.mediaquery", [newSize, currentSize]);
      }
    });
  }
};
...
var Foundation = {
  ...
  reInit: function(plugins2) {
    var isJQ = plugins2 instanceof $$1;
    try {
      if (isJQ) {
        plugins2.each(function() {
          $$1(this).data("zfPlugin")._init();
        });
      } else {
        var type = typeof plugins2, _this = this, fns = {
          "object": function(plgs) {
            plgs.forEach(function(p2) {
              p2 = hyphenate$1(p2);
              $$1("[data-" + p2 + "]").foundation("_init");
            });
          },
          "string": function() {
            plugins2 = hyphenate$1(plugins2);
            $$1("[data-" + plugins2 + "]").foundation("_init");
          },
          "undefined": function() {
  ...
  reflow: function(elem, plugins2) {
    ...
    var _this = this;
    $$1.each(plugins2, function(i2, name) {
      var plugin = _this._plugins[name];
      var $elem = $$1(elem).find("[data-" + name + "]").addBack("[data-" + name + "]").filter(function() {
        return typeof $$1(this).data("zfPlugin") === "undefined";
      });
      $elem.each(function() {
        var $el = $$1(this), opts = { reflow: true };
        ...
        try {
          $el.data("zfPlugin", new plugin($$1(this), opts));
        } catch (er) {
          console.error(er);
        } finally {
          return;
  ...
  addToJquery: function() {
    var foundation = function(method) {
      var type = typeof method, $noJS = $$1(".no-js");
      if ($noJS.length) {
...
function findFocusable($element) {
  ...
  return $element.find("a[href], area[href], input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), iframe, object, embed, *[tabindex], *[contenteditable]").filter(function() {
    if (!$$1(this).is(":visible") || $$1(this).attr("tabindex") < 0) {
      return false;
    }
    return true;
  }).sort(function(a2, b2) {
    if ($$1(a2).attr("tabindex") === $$1(b2).attr("tabindex")) {
      return 0;
    }
    let aTabIndex = parseInt($$1(a2).attr("tabindex"), 10), bTabIndex = parseInt($$1(b2).attr("tabindex"), 10);
    if (typeof $$1(a2).attr("tabindex") === "undefined" && bTabIndex > 0) {
      return 1;
    }
    if (typeof $$1(b2).attr("tabindex") === "undefined" && aTabIndex > 0) {
      return -1;
    }
...
var Keyboard = {
  ...
  handleKey(event2, component, functions) {
    ...
    if (typeof commandList.ltr === "undefined") {
      ...
      if (rtl()) cmds = $$1.extend({}, commandList.ltr, commandList.rtl);
      else cmds = $$1.extend({}, commandList.rtl, commandList.ltr);
    }
...
const Nest = {
  Feather(menu, type = "zf") {
    ...
    items.each(function() {
      var $item = $$1(this), $sub = $item.children("ul");
      if ($sub.length) {
...
function onTouchEnd(e2) {
  ...
  if (!didMoved) {
    var tapEvent = $$1.Event("tap", startEvent || e2);
    $$1(this).trigger(tapEvent);
  }
  startEvent = null;
  isMoving = false;
  didMoved = false;
...
function onTouchMove(e2) {
  if (true === $$1.spotSwipe.preventDefault) {
    e2.preventDefault();
  }
  if (isMoving) {
    ...
    if (Math.abs(dx) >= $$1.spotSwipe.moveThreshold && elapsedTime <= $$1.spotSwipe.timeThreshold) {
      dir = dx > 0 ? "left" : "right";
    }
    if (dir) {
      e2.preventDefault();
      onTouchEnd.apply(this, arguments);
      $$1(this).trigger($$1.Event("swipe", Object.assign({}, e2)), dir).trigger($$1.Event(`swipe${dir}`, Object.assign({}, e2)));
    }
  }
}
function onTouchStart(e2) {
  if (e2.touches.length === 1) {
    ...
    this.addEventListener("touchmove", onTouchMove, { passive: true === $$1.spotSwipe.preventDefault });
...
Touch$1.setupSpotSwipe = function() {
  $$1.spotSwipe = new SpotSwipe($$1);
};
Touch$1.setupTouchHandler = function() {
  $$1.fn.addTouch = function() {
    this.each(function(i2, el) {
      $$1(el).bind("touchstart touchmove touchend touchcancel", function(event2) {
        handleTouch(event2);
      });
    });
...
Touch$1.init = function() {
  if (typeof $$1.spotSwipe === "undefined") {
    Touch$1.setupSpotSwipe($$1);
    Touch$1.setupTouchHandler($$1);
  }
};
...
const triggers = (el, type) => {
  el.data(type).split(" ").forEach((id) => {
    $$1(`#${id}`)[type === "close" ? "trigger" : "triggerHandler"](`${type}.zf.trigger`, [el]);
  });
};
var Triggers = {
  Listeners: {
...
Triggers.Listeners.Basic = {
  openListener: function() {
    triggers($$1(this), "open");
  },
  closeListener: function() {
    let id = $$1(this).data("close");
    if (id) {
      triggers($$1(this), "close");
    } else {
      $$1(this).trigger("close.zf.trigger");
    }
  },
  toggleListener: function() {
    let id = $$1(this).data("toggle");
    if (id) {
      triggers($$1(this), "toggle");
    } else {
      $$1(this).trigger("toggle.zf.trigger");
    }
  },
  closeableListener: function(e2) {
    let animation = $$1(this).data("closable");
    e2.stopPropagation();
    if (animation !== "") {
      Motion.animateOut($$1(this), animation, function() {
        $$1(this).trigger("closed.zf");
      });
    } else {
      $$1(this).fadeOut().trigger("closed.zf");
    }
  },
  toggleFocusListener: function() {
    let id = $$1(this).data("toggle-focus");
    $$1(`#${id}`).triggerHandler("toggle.zf.trigger", [$$1(this)]);
  }
};
...
Triggers.Listeners.Global = {
  resizeListener: function($nodes) {
    if (!MutationObserver) {
      $nodes.each(function() {
        $$1(this).triggerHandler("resizeme.zf.trigger");
      });
    }
  ...
  scrollListener: function($nodes) {
    if (!MutationObserver) {
      $nodes.each(function() {
        $$1(this).triggerHandler("scrollme.zf.trigger");
      });
    }
  ...
  closeMeListener: function(e2, pluginId) {
    ...
    let plugins2 = $$1(`[data-${plugin}]`).not(`[data-yeti-box="${pluginId}"]`);
    plugins2.each(function() {
      let _this = $$1(this);
...
Triggers.Initializers.addClosemeListener = function(pluginName) {
  var yetiBoxes = $$1("[data-yeti-box]"), plugNames = ["dropdown", "tooltip", "reveal"];
  if (pluginName) {
  ...
  if (yetiBoxes.length) {
    ...
    $$1(window).off(listeners).on(listeners, Triggers.Listeners.Global.closeMeListener);
  }
};
function debounceGlobalListener(debounce, trigger, listener) {
  let timer, args = Array.prototype.slice.call(arguments, 3);
  $$1(window).on(trigger, function() {
    if (timer) {
      clearTimeout(timer);
    }
    timer = setTimeout(function() {
...
Triggers.Initializers.addResizeListener = function(debounce) {
  let $nodes = $$1("[data-resize]");
  if ($nodes.length) {
    debounceGlobalListener(debounce, "resize.zf.trigger", Triggers.Listeners.Global.resizeListener, $nodes);
  }
};
Triggers.Initializers.addScrollListener = function(debounce) {
  let $nodes = $$1("[data-scroll]");
  if ($nodes.length) {
...
Triggers.Initializers.addMutationEventsListener = function($elem) {
  ...
  var listeningElementsMutation = function(mutationRecordsList) {
    var $target = $$1(mutationRecordsList[0].target);
...
function onImagesLoaded(images, callback) {
  ...
  images.each(function() {
    if (this.complete && typeof this.naturalWidth !== "undefined") {
      ...
      $$1(image).one(events2, function me2() {
        $$1(this).off(events2, me2);
        singleImageLoaded();
      });
      image.src = $$1(this).attr("src");
    }
  });
...
class Dropdown extends Positionable {
  ...
  _setup(element, options) {
    this.$element = element;
    this.options = $$1.extend({}, Dropdown.defaults, this.$element.data(), options);
  ...
  _init() {
    ...
    this.$anchors = $$1(`[data-toggle="${$id}"]`).length ? $$1(`[data-toggle="${$id}"]`) : $$1(`[data-open="${$id}"]`);
    this.$anchors.attr({
      "aria-controls": $id,
      "data-is-focus": false,
      "data-yeti-box": $id,
   ...
   */
  _addBodyHandler() {
    var $body = $$1(document.body).not(this.$element), _this = this;
  ...
  _destroy() {
    ...
    $$1(document.body).off("click.zf.dropdown tap.zf.dropdown");
  }
}
Dropdown.defaults = {
  /**
...
class DropdownMenu extends Plugin {
  ...
  _setup(element, options) {
    this.$element = element;
    this.options = $$1.extend({}, DropdownMenu.defaults, this.$element.data(), options);
  ...
  _events() {
    ...
    var handleClickFn = function(e2) {
      var $elem = $$1(e2.target).parentsUntil("ul", `.${parClass}`), hasSub = $elem.hasClass(parClass), hasClicked = $elem.attr("data-is-click") === "true", $sub = $elem.children(".is-dropdown-submenu");
      if (hasSub) {
        if (hasClicked) {
    ...
    if (_this.options.closeOnClickInside) {
      this.$menuItems.on("click.zf.dropdownMenu", function() {
        var $elem = $$1(this), hasSub = $elem.hasClass(parClass);
        if (!hasSub) {
          _this._hide();
        }
      });
    ...
    this.$menuItems.on("keydown.zf.dropdownMenu", function(e2) {
      var $element = $$1(e2.target).parentsUntil("ul", '[role="none"]'), isTab = _this.$tabs.index($element) > -1, $elements = isTab ? _this.$tabs : $element.siblings("li").add($element), $prevElement, $nextElement;
      $elements.each(function(i2) {
        if ($$1(this).is($element)) {
  ...
  _addBodyHandler() {
    ...
    $body.on("click.zf.dropdownMenu tap.zf.dropdownMenu", (e2) => {
      var isItself = !!$$1(e2.target).closest(this.$element).length;
      if (isItself) return;
      this._hide();
      this._removeBodyHandler();
    });
   ...
   */
  _removeBodyHandler() {
    $$1(document.body).off("click.zf.dropdownMenu tap.zf.dropdownMenu");
  }
  /**
  ...
  _destroy() {
    ...
    $$1(document.body).off(".zf.dropdownMenu");
...
class Accordion extends Plugin {
  ...
  _setup(element, options) {
    this.$element = element;
    this.options = $$1.extend({}, Accordion.defaults, this.$element.data(), options);
  ...
  _init() {
    ...
    this.$tabs.each(function(idx, el) {
      var $el = $$1(el), $content = $el.children("[data-tab-content]"), id = $content[0].id || GetYoDigits(6, "accordion"), linkId = el.id ? `${el.id}-label` : `${id}-label`;
      ...
      if (isOwnAnchor) {
        ...
        if (this.options.deepLinkSmudge) {
          onLoad($$1(window), () => {
            ...
            $$1("html, body").animate({ scrollTop: offset.top - this.options.deepLinkSmudgeOffset }, this.options.deepLinkSmudgeDelay);
          });
        }
  ...
  _events() {
    ...
    if (this.options.deepLink) {
      $$1(window).on("hashchange", this._checkDeepLink);
    }
  }
  /**
  ...
  _destroy() {
    ...
    if (this.options.deepLink) {
      $$1(window).off("hashchange", this._checkDeepLink);
    }
  }
}
Accordion.defaults = {
...
class AccordionMenu extends Plugin {
  ...
  _setup(element, options) {
    this.$element = element;
    this.options = $$1.extend({}, AccordionMenu.defaults, this.$element.data(), options);
  ...
  _init() {
    ...
    this.$menuLinks.each(function() {
      var linkId = this.id || GetYoDigits(6, "acc-menu-link"), $elem = $$1(this), $sub = $elem.children("[data-submenu]"), subId = $sub[0].id || GetYoDigits(6, "acc-menu"), isActive = $sub.hasClass("is-active");
  ...
  _events() {
    ...
    this.$element.find("li").each(function() {
      var $submenu = $$1(this).children("[data-submenu]");
      if ($submenu.length) {
        if (_this.options.submenuToggle) {
          $$1(this).children(".submenu-toggle").off("click.zf.accordionMenu").on("click.zf.accordionMenu", function() {
            _this.toggle($submenu);
          });
        } else {
          $$1(this).children("a").off("click.zf.accordionMenu").on("click.zf.accordionMenu", function(e2) {
            e2.preventDefault();
            _this.toggle($submenu);
          });
        }
      ...
      var $element = $$1(this), $elements = $element.parent("ul").children("li"), $prevElement, $nextElement, $target = $element.children("[data-submenu]");
      $elements.each(function(i2) {
        if ($$1(this).is($element)) {
          $prevElement = $elements.eq(Math.max(0, i2 - 1)).find("a").first();
          $nextElement = $elements.eq(Math.min(i2 + 1, $elements.length - 1)).find("a").first();
          if ($$1(this).children("[data-submenu]:visible").length) {
            $nextElement = $element.find("li:first-child").find("a").first();
          }
          if ($$1(this).is(":first-child")) {
            $prevElement = $element.parents("li").first().find("a").first();
          } else if ($prevElement.parents("li").first().children("[data-submenu]:visible").length) {
            $prevElement = $prevElement.parents("li").find("li:last-child").find("a").first();
          }
          if ($$1(this).is(":last-child")) {
...
class OffCanvas extends Plugin {
  ...
  _setup(element, options) {
    ...
    this.$element = element;
    this.options = $$1.extend({}, OffCanvas.defaults, this.$element.data(), options);
    ...
    $$1(["push", "overlap"]).each((index, val) => {
      this.contentClasses.base.push("has-transition-" + val);
    });
    $$1(["left", "right", "top", "bottom"]).each((index, val) => {
  ...
  _init() {
    ...
    if (this.options.contentId) {
      this.$content = $$1("#" + this.options.contentId);
    ...
    this.$triggers = $$1(document).find('[data-open="' + id + '"], [data-close="' + id + '"], [data-toggle="' + id + '"]').attr("aria-expanded", "false").attr("aria-controls", id);
    this.position = this.$element.is(".position-left, .position-top, .position-right, .position-bottom") ? this.$element.attr("class").match(/position\-(left|top|right|bottom)/)[1] : this.position;
    if (this.options.contentOverlay === true) {
      var overlay = document.createElement("div");
      var overlayPosition = $$1(this.$element).css("position") === "fixed" ? "is-overlay-fixed" : "is-overlay-absolute";
      overlay.setAttribute("class", "js-off-canvas-overlay " + overlayPosition);
      this.$overlay = $$1(overlay);
      if (overlayPosition === "is-overlay-fixed") {
        $$1(this.$overlay).insertAfter(this.$element);
      } else {
  ...
  _events() {
    ...
    if (this.options.inCanvasOn) {
      $$1(window).on("changed.zf.mediaquery", () => {
        this._checkInCanvas();
      });
    }
  }
  ...
  _setMQChecker() {
    var _this = this;
    this.onLoadListener = onLoad($$1(window), function() {
      if (MediaQuery.atLeast(_this.options.revealOn)) {
        _this.reveal(true);
      }
    });
    $$1(window).on("changed.zf.mediaquery", function() {
      if (MediaQuery.atLeast(_this.options.revealOn)) {
        _this.reveal(true);
      } else {
        _this.reveal(false);
  ...
  _fixStickyElements() {
    this.$sticky.each((_2, el) => {
      ...
      if ($el.css("position") === "fixed") {
        ...
        let absoluteTopVal = $$1(document).scrollTop() + topVal;
  ...
  open(event2, trigger) {
    ...
    if (this.options.contentScroll === false) {
      $$1("body").addClass("is-off-canvas-open").on("touchmove", this._stopScrolling);
  ...
  close() {
    ...
    this.$element.one(transitionend(this.$element), () => {
      ...
      if (this.options.contentScroll === false) {
        $$1("body").removeClass("is-off-canvas-open").off("touchmove", this._stopScrolling);
  ...
  _destroy() {
    ...
    if (this.onLoadListener) $$1(window).off(this.onLoadListener);
  }
}
OffCanvas.defaults = {
  /**
...
class Tooltip extends Positionable {
  ...
  _setup(element, options) {
    this.$element = element;
    this.options = $$1.extend({}, Tooltip.defaults, this.$element.data(), options);
  ...
  _init() {
    ...
    this.template = this.options.template ? $$1(this.options.template) : this._buildTemplate(elemId);
  ...
  _buildTemplate(id) {
    ...
    var $template = $$1("<div></div>").addClass(templateClasses).attr({
      "role": "tooltip",
      "aria-hidden": true,
...
class SmoothScroll extends Plugin {
  ...
  _setup(element, options) {
    this.$element = element;
    this.options = $$1.extend({}, SmoothScroll.defaults, this.$element.data(), options);
   ...
   */
  _handleLinkClick(e2) {
    if (!$$1(e2.currentTarget).is('a[href^="#"]')) return;
...
class Sticky extends Plugin {
  ...
  _setup(element, options) {
    this.$element = element;
    this.options = $$1.extend({}, Sticky.defaults, this.$element.data(), options);
  ...
  _init() {
    ...
    if (this.options.anchor !== "") {
      $$1("#" + _this.options.anchor).attr({ "data-mutate": id });
    }
    ...
    this.onLoadListener = onLoad($$1(window), function() {
      ...
      if (_this.options.anchor !== "") {
        _this.$anchor = $$1("#" + _this.options.anchor);
      } else {
        _this._parsePoints();
      }
  ...
  _parsePoints() {
    ...
    for (var i2 = 0, len = pts.length; i2 < len && pts[i2]; i2++) {
      var pt2;
      if (typeof pts[i2] === "number") {
        pt2 = pts[i2];
      } else {
        var place = pts[i2].split(":"), anchor = $$1(`#${place[0]}`);
        pt2 = anchor.offset().top;
        if (place[1] && place[1].toLowerCase() === "bottom") {
          pt2 += anchor[0].getBoundingClientRect().height;
        }
      }
      breaks[i2] = pt2;
    }
  ...
  _events(id) {
    ...
    if (this.canStick) {
      this.isOn = true;
      $$1(window).off(scrollListener).on(scrollListener, function() {
  ...
  _destroy() {
    ...
    if (this.scrollListener) $$1(window).off(this.scrollListener);
    if (this.onLoadListener) $$1(window).off(this.onLoadListener);
    if (this.wasWrapped) {
      this.$element.unwrap();
    } else {
...
class Toggler extends Plugin {
  ...
  _setup(element, options) {
    this.$element = element;
    this.options = $$1.extend({}, Toggler.defaults, element.data(), options);
    this.className = "";
   ...
   */
  _init() {
    var id = this.$element[0].id, $triggers = $$1(`[data-open~="${id}"], [data-close~="${id}"], [data-toggle~="${id}"]`);
    var input;
  ...
  _updateARIA(isOn) {
    ...
    $$1(`[data-open="${id}"], [data-close="${id}"], [data-toggle="${id}"]`).attr({
...
class Equalizer extends Plugin {
  ...
  _setup(element, options) {
    this.$element = element;
    this.options = $$1.extend({}, Equalizer.defaults, this.$element.data(), options);
  ...
  _init() {
    ...
    if (this.options.equalizeOn) {
      ...
      $$1(window).on("changed.zf.mediaquery", this._checkMQ.bind(this));
    } else {
      this._events();
    }
  ...
  getHeights(cb) {
    var heights = [];
    for (var i2 = 0, len = this.$watched.length; i2 < len; i2++) {
      this.$watched[i2].style.height = "auto";
      heights.push(this.$watched[i2].offsetHeight);
    }
    cb(heights);
  }
  /**
  ...
  getHeightsByRow(cb) {
    ...
    for (var i2 = 0, len = this.$watched.length; i2 < len; i2++) {
      this.$watched[i2].style.height = "auto";
      var elOffsetTop = $$1(this.$watched[i2]).offset().top;
      if (elOffsetTop !== lastElTopOffset) {
        group++;
        groups[group] = [];
        lastElTopOffset = elOffsetTop;
      }
      groups[group].push([this.$watched[i2], this.$watched[i2].offsetHeight]);
    }
    for (var j2 = 0, ln = groups.length; j2 < ln; j2++) {
      var heights = $$1(groups[j2]).map(function() {
        return this[1];
      }).get();
  ...
  applyHeightByRow(groups) {
    this.$element.trigger("preequalized.zf.equalizer");
    for (var i2 = 0, len = groups.length; i2 < len; i2++) {
      var groupsILength = groups[i2].length, max = groups[i2][groupsILength - 1];
      if (groupsILength <= 2) {
        $$1(groups[i2][0][0]).css({ "height": "auto" });
        continue;
      }
      this.$element.trigger("preequalizedrow.zf.equalizer");
      for (var j2 = 0, lenJ = groupsILength - 1; j2 < lenJ; j2++) {
        $$1(groups[i2][j2][0]).css({ "height": max });
      }
...
class Abide extends Plugin {
  ...
  _setup(element, options = {}) {
    this.$element = element;
    this.options = $$1.extend(true, {}, Abide.defaults, this.$element.data(), options);
    this.isEnabled = true;
  ...
  _init() {
    ...
    if (this.options.a11yAttributes) {
      this.$inputs.each((i2, input) => this.addA11yAttributes($$1(input)));
      $globalErrors.each((i2, error) => this.addGlobalErrorA11yAttributes($$1(error)));
    }
    this._events();
  }
  /**
  ...
  _events() {
    ...
    this.$submits.off("click.zf.abide keydown.zf.abide").on("click.zf.abide keydown.zf.abide", (e2) => {
      if (!e2.key || (e2.key === " " || e2.key === "Enter")) {
        e2.preventDefault();
        this.formnovalidate = e2.target.getAttribute("formnovalidate") !== null;
        this.$element.submit();
      }
    });
    if (this.options.validateOn === "fieldChange") {
      this.$inputs.off("change.zf.abide").on("change.zf.abide", (e2) => {
        this.validateInput($$1(e2.target));
      });
    }
    if (this.options.liveValidate) {
      this.$inputs.off("input.zf.abide").on("input.zf.abide", (e2) => {
        this.validateInput($$1(e2.target));
      });
    }
    if (this.options.validateOnBlur) {
      this.$inputs.off("blur.zf.abide").on("blur.zf.abide", (e2) => {
        this.validateInput($$1(e2.target));
      });
    }
  }
  ...
  findFormError($el, failedValidators) {
    ...
    if (!!failedValidators) {
      ...
      failedValidators.forEach((v2) => {
        $error = $error.add($el.siblings(`[data-form-error-on="${v2}"]`));
        $error = $error.add(this.$element.find(`[data-form-error-for="${id}"][data-form-error-on="${v2}"]`));
      });
    }
    return $error;
  }
   ...
   */
  findRadioLabels($els) {
    var labels = $els.map((i2, el) => {
      var id = el.id;
      var $label = this.$element.find(`label[for="${id}"]`);
      if (!$label.length) {
        $label = $$1(el).closest("label");
      }
      return $label[0];
    });
    return $$1(labels);
  ...
  addA11yAttributes($el) {
    ...
    if ($labels.filter("[for]").length < $labels.length) {
      ...
      $labels.each((i2, label) => {
        const $label = $$1(label);
        if (typeof $label.attr("for") === "undefined")
          $label.attr("for", elemId);
      });
    }
    $errors.each((i2, label) => {
      const $label = $$1(label);
  ...
  validateInput($el) {
    ...
    if (validator) {
      ...
      validator.split(" ").forEach((v2) => {
        this.options.validators[v2]($el, required, $el.parent()) || failedValidators.push(v2);
      });
    }
  ...
  validateForm() {
    ...
    this.$inputs.each(function() {
      if ($$1(this)[0].type === "checkbox") {
        if ($$1(this).attr("name") === checkboxGroupName) return true;
        checkboxGroupName = $$1(this).attr("name");
      }
      acc.push(_this.validateInput($$1(this)));
    });
    var noError = acc.indexOf(false) === -1;
    this.$element.find("[data-abide-error]").each((i2, elem) => {
      const $elem = $$1(elem);
  ...
  validateCheckbox(groupName) {
    ...
    if (!valid) {
      $group.each((i2, e2) => {
        ...
        if (typeof $$1(e2).attr("data-min-required") !== "undefined") {
          minRequired = parseInt($$1(e2).attr("data-min-required"), 10);
        }
      });
    ...
    $group.each((i2, e2) => {
      if (!valid) {
        this.addErrorClasses($$1(e2), ["required"]);
      } else {
        this.removeErrorClasses($$1(e2));
      }
    });
    return valid;
  }
  ...
  matchValidation($el, validators, required) {
    ...
    var clear = validators.split(" ").map((v2) => {
      return this.options.validators[v2]($el, required, $el.parent());
    });
  ...
  resetForm() {
    ...
    $$1(`.${opts.labelErrorClass}`, $form).not("small").removeClass(opts.labelErrorClass);
    $$1(`.${opts.inputErrorClass}`, $form).not("small").removeClass(opts.inputErrorClass);
    $$1(`${opts.formErrorSelector}.${opts.formErrorClass}`).removeClass(opts.formErrorClass);
    $form.find("[data-abide-error]").css("display", "none");
    $$1(":input", $form).not(":button, :submit, :reset, :hidden, :radio, :checkbox, [data-abide-ignore]").val("").attr({
      "data-invalid": null,
      "aria-invalid": null
    });
    $$1(":input:radio", $form).not("[data-abide-ignore]").prop("checked", false).attr({
      "data-invalid": null,
      "aria-invalid": null
    });
    $$1(":input:checkbox", $form).not("[data-abide-ignore]").prop("checked", false).attr({
      "data-invalid": null,
      "aria-invalid": null
    });
  ...
  _destroy() {
    ...
    this.$inputs.off(".abide").each(function() {
      _this.removeErrorClasses($$1(this));
    });
...
Abide.defaults = {
  ...
  validators: {
    equalTo: function(el) {
      return $$1(`#${el.attr("data-equalto")}`).val() === el.val();
    }
  }
};
...
const initFoundation = () => $$1(document).foundation();
if ("requestIdleCallback" in window) {
  requestIdleCallback(initFoundation, { timeout: 2e3 });
} else {
  setTimeout(initFoundation, 100);
}
